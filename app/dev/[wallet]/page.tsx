...
type DevPayload = {
  ...
  coins: { id: string; wallet: string; token_address: string; title: string | null; description: string | null; created_at: string }[];
};

export default function DevPublicPage({ params }: { params: { wallet: string } }) {
  const devWallet = params.wallet;
  ...
  async function load() {
    setErr(null);
    const res = await fetch(`/api/public/dev/${encodeURIComponent(devWallet)}`);
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            className={[
              ...
              data?.isFollowing ? "bg-white/10 text-white hover:bg-white/15 border border-white/10" : "bg-white text-black hover:bg-zinc-200"
            ].join(" ")}
          >
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="flex items-center gap-4">
                <div className="h-14 w-14 overflow-hidden rounded-full border border-white/10 bg-white/5">
                  ...
                  {data.profile.pfp_url ? <img src={data.profile.pfp_url} alt="" className="h-full w-full object-cover" /> : null}
                </div>
