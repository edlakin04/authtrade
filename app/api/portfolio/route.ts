...
export async function GET(req: Request) {
  try {
    ...
    // SOL balance
    const lamports: number = await rpc(origin, "getBalance", [owner, { commitment: "confirmed" }]);
    ...
    // Keep non-zero only, sort by amount (rough)
    const nonZero = tokens.filter((t: any) => t.uiAmount > 0);

    // Include WSOL mint in price fetch so SOL can get USD too
    const WSOL_MINT = "So11111111111111111111111111111111111111112";
    const ids = [WSOL_MINT, ...nonZero.map((t: any) => t.mint)]
      .filter(Boolean)
      .slice(0, 50); // Jupiter price API v3 supports up to 50 ids per request :contentReference[oaicite:1]{index=1}
    ...
    const tokenRows = nonZero.map((t: any) => {
    ...
    const totalUsd =
      (solUsdValue ?? 0) +
      tokenRows.reduce((sum: number, r: any) => sum + (r.usdValue ?? 0), 0);
